...
describe('addition of a new blog', () => {
  test('succeeds with valid data', async () => {
    const loginResponse = await api.post('/api/login').send(helper.testUserLogin)
    await api
      .post('/api/blogs')
  ...
  test('fails with status code 400 if title is missing', async () => {
    ...
    await api.post('/api/blogs').set('Authorization', 'Bearer ' + loginResponse.body.token).send(newBlog).expect(400)
    const blogsAtEnd = await api.get('/api/blogs')
...
describe('deletion of a blog', () => {
  test('is successful with status 204 if valid id is given and user is correct', async () => {
    ...
    await api.delete(`/api/blogs/${blogToDelete.id}`).set('Authorization', 'Bearer ' + loginResponse.body.token).expect(204)
  ...
  test('fails with 404 if id does not exist in the database', async () => {
    ...
    await api.delete(`/api/blogs/${invalidId}`).set('Authorization', 'Bearer ' + loginResponse.body.token).expect(404)
  })
  test('fails with 401 if user is different', async () => {
    ...
    await user.save()
    const loginResponseUkko = await api.post('/api/login').send({ username: 'testiukko', password: 'salasana'})
    await api.delete(`/api/blogs/${blogToDelete.id}`).set('Authorization', 'Bearer ' + loginResponseUkko.body.token).expect(401)
  })
})
